...
class Response {
    ...
    set type(type) {
        type = utils_1.CacheContentTypes(type);
        if (type) {
    ...
    set body(val) {
        ...
        if ('function' == typeof val.pipe) {
            onFinish(this.res, utils_1.StreamDestroy.bind(null, val));
            utils_1.ErrorInject(val, (err) => this.ctx.onerror(err));
    ...
    redirect(url, alt) {
        if ('back' == url)
            url = this.ctx.request.get('Referrer') || alt || '/';
